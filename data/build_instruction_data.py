...
def load_curated_examples(source_root):
    ...
    for line_number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        record = make_record(
            raw["instruction"], raw["input"], raw["response"], category,
...
def main():
    ...
    if written_total != len(supervised):
        raise RuntimeError(f"Generated supervised dataset count mismatch: built {len(supervised)}, wrote {written_total}")
