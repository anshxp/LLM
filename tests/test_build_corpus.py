...
def test_build_corpus_handles_nested_files_and_content_duplicates(tmp_path, monkeypatch):
    raw = tmp_path / "raw"
    ...
    assert len((processed / "corpus.txt").read_text(encoding="utf-8").strip()) > 0
    assert len((processed / "manifest.jsonl").read_text(encoding="utf-8").splitlines()) == 1
