...
def parse_args(args=None):
    ...
    parser.add_argument(
        "--dataset", choices=("base", "healthcare", "instruction"), default="base"
    )
    ...
    parser.add_argument(
        "--instruction-dir", type=Path, default=DEFAULT_INSTRUCTION_DIR
    )
    parser.add_argument(
        "--instruction-categories", default=DEFAULT_INSTRUCTION_CATEGORIES
    )
    ...
    if parsed.learning_rate is None:
        parsed.learning_rate = (
            ...
            if parsed.dataset == "instruction"
            else DEFAULT_LEARNING_RATE
        )
    if parsed.lr_min is None:
        parsed.lr_min = (
            DEFAULT_SFT_LR_MIN if parsed.dataset == "instruction" else DEFAULT_LR_MIN
        )
    ...
    parsed.instruction_categories = tuple(
        category.strip()
        for category in parsed.instruction_categories.split(",")
        if category.strip()
    )
    return parsed
...
def build_dataset(
    ...
    if dataset_name == "instruction":
        ...
        if list(instruction_dir.glob(f"{split}-*.jsonl")):
            return ShardedInstructionDataset(
                instruction_dir,
                split,
                context_length,
